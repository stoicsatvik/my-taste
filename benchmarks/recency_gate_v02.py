...
def main():
    with tempfile.TemporaryDirectory() as tmp:
        engine = ContextTasteEngine(Path(tmp) / "recency.sqlite", context_retention=CONTEXT_RETENTION)
        for _ in range(8): engine.observe_choice(OLD_PREFERRED, OLD_REJECTED, context="chat")
        for _ in range(8): engine.observe_choice(STABLE_PREFERRED, STABLE_REJECTED, context="proposal")
        ...
        result = {"fixture":"synthetic-context-reversal-v1","challenger":"bounded-context-recency-v1","context_retention":CONTEXT_RETENTION,"pre":pre,"adaptation_step":adaptation_step,"post":post,"stable_before":stable_before,"stable_after":stable_after,"trajectory":trajectory}
