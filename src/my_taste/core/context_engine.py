...
class ContextTasteEngine(TasteEngine):
    ...
    The v0.1 global model remains intact. When a non-empty context is supplied,
    an additional context-scoped weight vector is learned and added to the global
    vector at scoring time. This makes contextual preferences explicit without
    silently changing unrelated contexts.
    """
    ...
    def get_context_weights(self, domain: str, context: str) -> dict[str, PreferenceWeight]:
        ...
        with self._connect() as conn:
            rows = conn.execute(
                ...
                FROM context_preference_weights
                WHERE domain = ? AND context = ?
                """,
                (domain, key),
            ).fetchall()
        return {
    ...
    def _update_context_weights(self, domain: str, context: str, deltas: dict[str, float]) -> None:
        ...
        with self._connect() as conn:
            for feature, delta in deltas.items():
                conn.execute(
                    """
                    INSERT INTO context_preference_weights
                    (domain, context, feature, weight, updates, updated_at)
                    VALUES (?, ?, ?, ?, 1, ?)
                    ...
                    """,
                    (domain, key, feature, float(delta), now),
                )
    ...
    def _score_contextual_features(
        self,
    ...
    def context_profile(
        ...
        *,
        ...
        features = set(global_weights) | set(local_weights)
        rows: list[dict[str, object]] = []
        for feature in features:
            global_weight = global_weights.get(feature)
            local_weight = local_weights.get(feature)
            effective = (global_weight.weight if global_weight else 0.0) + (
                local_weight.weight if local_weight else 0.0
            )
            updates = max(
                global_weight.updates if global_weight else 0,
                local_weight.updates if local_weight else 0,
            )
            confidence = 1.0 - math.exp(-updates / 5.0)
            rows.append(
                {
                    "feature": feature,
                    "direction": "prefer_more" if effective > 0 else "prefer_less",
                    "effective_weight": effective,
                    "global_weight": global_weight.weight if global_weight else 0.0,
                    "context_weight": local_weight.weight if local_weight else 0.0,
        ...
        return [
            {
                ...
                "preferred": item.payload.get("preferred"),
                "rejected": item.payload.get("rejected"),
            }
            for item in evidence[: max(1, min(limit, 200))]
        ]
    ...
    def taste_context(
        ...
        *,
        ...
        return {
            ...
            "instructions": [
                preference_instruction(str(item["feature"]), str(item["direction"]))
                for item in profile
                if float(item["confidence"]) >= 0.15
            ],
            "profile": profile,
